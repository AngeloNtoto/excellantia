...
const DIFFICULTIES = [
  {
    value: "EASY",
    label: "Facile",
    desc: "Majorité de questions fondamentales",
    icon: TrendingDown,
    color: "text-emerald-600 dark:text-emerald-400",
    bg: "bg-emerald-50 dark:bg-emerald-500/10",
  ...
  },
  {
    value: "MIXED",
    label: "Mixte",
    desc: "Mélange équilibré (standard)",
    icon: Gauge,
    color: "text-indigo-600 dark:text-indigo-400",
    bg: "bg-indigo-50 dark:bg-indigo-500/10",
  ...
  },
  {
    value: "HARD",
    label: "Difficile",
    desc: "Questions complexes et avancées",
    icon: TrendingUp,
...
export function TrainingForm() {
  ...
  const [chronoMode, setChronoMode] = useState<"GALILEE" | "HEISENBERG" | "SCHRODINGER">(
    "GALILEE"
  );
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const finalDuration =
      duration === "" || Number(duration) < 1 ? 100 : Number(duration);
    ...
    SUBJECTS.forEach((subj) => {
      ...
      fd.append(
        key,
        selectedSubjects.includes(subj.id) ? String(subjectCounts[subj.id] || 0) : "0"
      );
    });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                ...
                return (
                  <label
                    key={mode.value}
                    className={`cursor-pointer flex items-start gap-2.5 p-3 rounded-xl border-2 transition-all ${
                      selected
                  ...
                  >
                    <input
                      type="radio"
                      name="chronoModeChoice"
                      value={mode.value}
                      checked={selected}
                      onChange={() => setChronoMode(mode.value as typeof chronoMode)}
                      className="sr-only"
                    />
                    <Icon
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <input
                type="radio"
                name="pausableTimer"
                checked={!pausableTimer}
                onChange={() => setPausableTimer(false)}
                className="sr-only"
              />
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Le temps continue de s'écouler si vous fermez la page.
                </p>
              </div>
            </label>
            ...
            >
              <input
                ...
                name="pausableTimer"
                checked={pausableTimer}
                onChange={() => setPausableTimer(true)}
                className="sr-only"
              />
              <div className="p-2 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 shrink-0">
              ...
              <div>
                <p className="font-bold text-xs sm:text-sm text-slate-900 dark:text-white">
                  Relatif (Pause active)
                </p>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Le chronomètre se fige si vous quittez la session.
                </p>
              </div>
            </label>
          </div>
        ...
        >
          {isPending ? (
            ...
            <>
              ...
              <span>
                Lancer l'entraînement ({totalQuestions} questions • {duration} min)
              </span>
            </>
          )}
        </button>
